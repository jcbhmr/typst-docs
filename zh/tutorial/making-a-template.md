...
![Preview](/assets-docs/f55a5389282e58c6cefb7ae2329cc761.png)
...
![Preview](/assets-docs/c9ce0a0f4a0e86c783e77271ca2f92a9.png)
...
![Preview](/assets-docs/6e34fb6913212950a231d345d81c51f0.png)
...
![Preview](/assets-docs/6e9bfdb0ecedf2ac3a531b167f29927f.png)
...
![Preview](/assets-docs/bd89fabae6b6ddeaf18c48720c8c077f.png)

我们复制粘贴了上一章中的大部分代码。 唯一的两个区别是，我们将所有内容都包装在函数 `conf` 中， 并直接在 `doc` 参数上调用 `columns` 函数，因为 `doc` 对应着整个文档的内容。 此外，我们使用大括号代码块而不是内容块。 这样，我们就不需要为所有的 Set 规则和函数调用加上 `#` 前缀。 与之相对的，我们也不再能再直接在里面写标记文本了。
...
新的 `authors` 参数接收一个由 [字典](/zh/reference/foundations/dictionary/) 的 [数组](/zh/reference/foundations/array/)，其中带有键 `name`、`affiliation` 和 `email`。 因为我们可以输入任意数量的作者，所以我们需要动态地确定作者列表是需要一列、两列还是三列。 首先，我们在 `authors` 数组上使用 [`.len()`](/zh/reference/foundations/array/#definitions-len) 方法确定作者的数量。 然后，我们将列数设置为作者数量和 3 之间的最小值，以便我们永远不会创建超过三个列。 如果作者超过三个，则将插入一个新行。为此，我们还在 `grid` 函数中添加了一个 `row-gending` 参数。 否则，这些行将会靠得太近。为了从字典中提取有关作者的详细信息，我们使用 [字段访问语法](/zh/reference/scripting/#fields)。

我们仍然必须为每个作者对应的网格提供一个参数：这就是数组的 [`map` 方法](/zh/reference/foundations/array/#definitions-map) 派上用场的地方。 它将一个函数作为参数，该函数与数组的每个项一起调用。 我们给它传递一个函数，该函数会格式化每个作者的详细信息，并返回一个包含内容值的新数组。 现在，我们有一个值数组，我们希望将其用作网格的多个参数。 我们可以通过使用 [`spread` 操作符](/zh/reference/foundations/arguments/) 来做到这一点。 它接受一个数组，并将其数组里的每个项目作为单独的参数应用于函数中。
...
![Preview](/assets-docs/bb3339dcb394a58ead83735587d342bd.png)
